package com.myq.preloadmanager.cache;


import android.util.Log;

import com.myq.preloadmanager.HttpProxyCacheServer;
        ...
        try {
            ...
            while ((length = in.read(bytes)) != -1) {
                ...
                if (mIsCanceled || read >= PreloadManager.PRELOAD_LENGTH) {
                    if (mIsCanceled) {
                        Log.i("myq","预加载取消：" + mPosition + " 读取数据：" + read + " Byte");
                    } else {
                        Log.i("myq","预加载成功：" + mPosition + " 读取数据：" + read + " Byte");
                    }
                    break;
                }
            }
        } catch (Exception e) {
            Log.i("myq","预加载异常：" + mPosition + " 异常信息："+ e.getMessage());
            // 关入小黑屋
